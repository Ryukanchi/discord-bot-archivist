...
class ServerArchivist {
    constructor() {
        try {
            ...
            this.highlightThresholds = {
                ...
                reactions: parseInt(process.env.REACTION_THRESHOLD) || 3,
                replies: parseInt(process.env.REPLY_THRESHOLD) || 2,
                keywords: process.env.KEYWORDS ? process.env.KEYWORDS.split(',').map(k => k.trim().toLowerCase()) : ['lol', 'haha', 'omg', 'wtf', 'epic', 'amazing', 'wow'],
    ...
    addUserPoints(userId, points) {
        ...
        `);
        stmt.run(hashedUserId, hashedUserId, points);
    }

    getUserPoints(userId) {
        const hashedUserId = this.hashUserId(userId);
        const stmt = this.db.prepare('SELECT * FROM user_points WHERE user_id = ?');
        return stmt.get(hashedUserId) || { user_id: hashedUserId, points: 0, highlights_created: 0, votes_cast: 0 };
    }
